...
export function NoteCard({ note, canEdit = true }: NoteCardProps) {
  return (
    <Card
      className="group relative mb-3 break-inside-avoid rounded-xl border border-white/[0.07] p-4
                 transition-all duration-200 hover:-translate-y-0.5 hover:border-white/[0.10]
    ...
    >
      ...
      {note.pinned ? <Pin className="mb-2 h-3.5 w-3.5 text-amber-400" /> : null}

      <h3 className="mb-2 pr-16 text-sm font-medium text-white line-clamp-1">{note.title}</h3>

      <p className="line-clamp-5 text-xs leading-relaxed text-white/50">{note.content?.slice(0, 180) || "Без текста"}</p>
